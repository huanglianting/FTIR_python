...
def plot_spectrum_with_marked_peaks(x, spectrum_1, spectrum_2, save_path, peak_wavenumbers):
    ...
    spectrum_data.to_csv(os.path.join(
        save_path, 'spectrum_input_data.csv'), index=False)
    # 保存峰位数据
    peak_data = pd.DataFrame({'peak_wavenumbers': peak_wavenumbers})
    peak_data.to_csv(os.path.join(
        save_path, 'peak_wavenumbers.csv'), index=False)
    ...
    sem_1 = np.std(spectrum_1, axis=1) / np.sqrt(spectrum_1.shape[1])

    mean_2 = np.mean(spectrum_2, axis=1)
    std_2 = np.std(spectrum_2, axis=1)
    sem_2 = np.std(spectrum_2, axis=1) / np.sqrt(spectrum_2.shape[1])
