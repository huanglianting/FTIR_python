...
# 读取训练集和测试集
X_train_scaled = np.load(f"{save_path}/X_train_mz.npy")
X_test_scaled = np.load(f"{save_path}/X_test_mz.npy")
y_train = np.load(f"{save_path}/y_train_mz.npy")
y_test = np.load(f"{save_path}/y_test_mz.npy")


'''
# 训练 PCA-LDA 模型并保存模型参数，只需run一次，后续测试会自己读取train保存的参数
train_pca_lda_model(X_train_scaled, y_train, save_path, n_pca_components=20) #这里报错， The number of samples must be more than the number of classes.
